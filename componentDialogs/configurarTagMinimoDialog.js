...
class ConfigurarTagMinimoDialog extends ComponentDialog {

    constructor(conservsationState, userState) {
        super('configurarTagMinimoDialog');
        ...
        this.addDialog(new WaterfallDialog(WATERFALL_DIALOG, [
            ...
            this.configurarMinimo.bind(this)
        ]));
    ...
    async obterValor(step) {
        ...
        if ('valor' in step._info.options) {
            ...
            return await step.prompt(NUMBER_PROMPT, await this.translator.t('Qual o valor para setar de mínimo?'));
        }
    ...
    async confirmarValores(step) {
        ...
        return await step.prompt(CONFIRM_PROMPT, await this.translator.t('Confirma configuração de valor do mínimo?'), ['yes', 'no']);
    }

    async configurarMinimo(step) {
        if (step.result === true) {
            ...
            sql += " set minimo= " + step.values.valor;
...
module.exports.ConfigurarTagMinimoDialog = ConfigurarTagMinimoDialog;